...
const unsigned long MOTOR_RUN_TIME = 3000; // ms motor spins on a win

// ---------------- RAM diagnostic (optional - remove once confirmed fixed) ----------------
int freeRam() {
...
void showEntryScreen(int digit) {
  ...
  display.println(F("Enter number 1-6"));
...
void showInvalidEntry() {
  ...
  display.println(F("Invalid! Enter a"));
  display.println(F("number from 1 to 6"));
  display.display();
  tone(buzzer, 300, 400);
  delay(1200);
}
